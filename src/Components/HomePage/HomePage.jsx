  ...
  const handleConfirmRoute = useCallback(
    async (fromLocation, toDestination) => {
      try {
        ...
        setTimeout(() => {
          setShowRoute(true);
          if (isMobile) setIsFormCollapsed(true); // ← collapse after confirm
        }, 100);
      } catch (error) {
      ...
      {!(isMobile && showNotifications) && (
        <div className={`${showForm ? "form-view" : "delivery-view"}`}>
          <div className="Components-container">
            ...
            >
              <div
                className={`Components-container1 ${selectedDelivery ? "panel-open" : ""} ${
                  isInfoPanelOpen ? "expanded" : ""
                } ${isMobile && showForm && isFormCollapsed ? "form-collapsed" : ""}`}
              >
                {isMobile && showForm && (
                  <div
                    className={`form-sheet-handle ${isFormCollapsed ? "collapsed" : ""}`}
                    onClick={() => setIsFormCollapsed((prev) => !prev)}
                  >
                    <div className="handle-bar" />
                    {isFormCollapsed && (
                      <div className="collapsed-preview">
                        <span>{filterCriteria.location}</span>
                        <span className="arrow-divider">→</span>
                        <span>{filterCriteria.destination}</span>
                      </div>
                    )}
                  </div>
                )}
